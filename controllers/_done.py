...
class OdooRPCController(http.Controller):
    ...
    def _fetch_full_objects_with_fields(self, env, model_name, ids, desired_fields=None, options=None):
        ...
        try:
            ...
            # NEW: Apply follower filter if requested in options
            if options.get('check_message_follower_ids'):
                user_partner_id = env.user.partner_id.id if env.user.partner_id else None
                if user_partner_id:
                    try:
                        model_for_check = env[model_name]
                        if 'message_follower_ids' in model_for_check.fields_get():
                            _logger.info(f"Applying follower filter to {len(ids)} IDs for model {model_name}")
                            domain = [('id', 'in', ids), ('message_follower_ids.partner_id', '=', user_partner_id)]
                            filtered_ids = model_for_check.search(domain).ids
                            _logger.info(f"Follower filter reduced IDs from {len(ids)} to {len(filtered_ids)}")
                            ids = filtered_ids  # Replace original IDs with filtered ones
                        else:
            ...
            if not ids:  # If filtering left no IDs, return early
                return []
