...
class OdooRPCController(http.Controller):
    ...
    def _authenticate(self, db, username, password):
        ...
        try:
            ...
            try:
                ...
                # Get employee info
                employee = user.employee_id if hasattr(user, 'employee_id') else None
                ...
                user_info = {
                    ...
                    "case_id": employee.id if employee and employee.id else None,
                    "balance": employee.id if employee and employee.id else None,
        ...
        try:
            ...
            if '.' in field_path:
                ...
                if field_path in record and record[field_path]:
                    ...
                    target_model, desired_fields, options = self._parse_target_config(target_config, full_path)
                    ...
                    if ids_to_fetch:
                        ...
                        try:
                            ...
                            target_objects = self._fetch_full_objects_with_fields(env, target_model, ids_to_fetch,
                                                                                  desired_fields, options)

                            # Replace original value. An empty list is a valid result of filtering.
                            if isinstance(field_value, (int, str)):
                                # For a single ID, replace with the single object or None
                                record[field_path] = target_objects[0] if target_objects else None
                                _logger.info(f"✅ Replaced single ID with object (or None) for '{full_path}'")
                            else:
                                # For a list of IDs, replace with the list of objects (can be empty)
                                record[field_path] = target_objects
                                _logger.info(f"✅ Replaced list of IDs with {len(target_objects)} objects for '{full_path}'")
    ...
    def _parse_target_config(self, target_config, field_path):
        """Parse target configuration to extract model name, desired fields, and options."""
        try:
            if isinstance(target_config, dict):
                model_name = None
                desired_fields = None
                
                # Find the model key (e.g., "project.project") which has a list of fields as value
                for key, value in target_config.items():
                    if '.' in key and isinstance(value, list):
                        model_name = key
                        desired_fields = value
                        break
                
                # Fallback for old format or if model key not found yet
                if not model_name and len(target_config) == 1 and isinstance(list(target_config.values())[0], list):
                    model_name = list(target_config.keys())[0]
                    desired_fields = list(target_config.values())[0]

                if not model_name:
                    _logger.error(f"Invalid target config format for '{field_path}': Could not find model definition in {target_config}")
                    return None, None, {}

                # The rest of the keys are options
                options = {k: v for k, v in target_config.items() if k != model_name}
                
                return model_name, desired_fields, options
    ...
    def _fetch_full_objects_with_fields(self, env, model_name, ids, desired_fields=None, options=None):
        """Fetch full objects from specified model with custom field selection and options."""
        if options is None:
            options = {}
        try:
            ...
            # Get model object early to check fields and perform search
            try:
            ...
            # Start with a domain to filter by the provided IDs
            combined_domain = [('id', 'in', ids)]

            # Apply follower filter if requested in options
            if options.get('check_message_follower_ids'):
                user_partner_id = env.user.partner_id.id if env.user.partner_id else None
                if user_partner_id:
                    try:
                        # Special handling for project.project model using privacy_visibility
                        if model_name == 'project.project':
                            _logger.info(f"Applying privacy_visibility filter for {model_name}")
                            # Filter projects that are either:
                            # 1. Visible to followers AND user is a follower
                            # 2. Visible to all employees (if user is an employee)
                            # 3. Visible to portal users (if applicable)
                            
                            # Check if user is employee
                            is_employee = env.user.has_group('base.group_user')
                            
                            if is_employee:
                                # For employees: show follower projects + employee visible projects
                                privacy_domain = [
                                    '|',
                                    '&',
                                    ('privacy_visibility', '=', 'followers'),
                                    ('message_follower_ids.partner_id', '=', user_partner_id),
                                    ('privacy_visibility', '=', 'employees')
                                ]
                            else:
                                # For portal/public users: only follower projects or portal visible
                                privacy_domain = [
                                    '|',
                                    '&',
                                    ('privacy_visibility', '=', 'followers'),
                            ...
                            combined_domain.extend(privacy_domain)
                            _logger.info(f"Applied privacy_visibility filter. New domain: {combined_domain}")
                        
                        # Standard message_follower_ids filter for other models
                        elif 'message_follower_ids' in model_obj.fields_get():
                            _logger.info(f"Applying follower filter for {model_name}")
                            combined_domain.append(('message_follower_ids.partner_id', '=', user_partner_id))
                        else:
                            _logger.warning(f"Cannot apply follower filter: model {model_name} has no 'message_follower_ids' field.")
                    except Exception as filter_error:
                        _logger.error(f"Error applying follower filter for {model_name}: {filter_error}")
                else:
                    _logger.warning(f"User has no partner_id, cannot apply follower filter for {model_name}.")

            # Apply additional domain filter if present in options
            if options.get('domain'):
                option_domain = options.get('domain')
                if isinstance(option_domain, list):
                    combined_domain.extend(option_domain)
                    _logger.info(f"Applying additional domain filter for {model_name}: {option_domain}")
                else:
                    _logger.warning(f"Invalid domain format in options for {model_name}: {option_domain}")

            # Perform the search with the combined domain
            filtered_records = model_obj.search(combined_domain)
            
            if not filtered_records:
                _logger.warning(f"No records found in '{model_name}' for combined domain: {combined_domain}")
                return []
            ...
            # Read data from the filtered records
            object_data = filtered_records.read(available_fields)
    ...
    def _handle_rpc(self, data):
        ...
        try:
            ...
            try:
                ...
                # Handle check_message_follower_ids for search/search_read
                if kwargs.get('check_message_follower_ids') and user_partner_id and method in ['search', 'search_read']:
                    _logger.info(f"🔬 check_message_follower_ids is TRUE for {model}.{method}")
                    try:
                        model_for_check = env[model]
                        
                        # Special handling for project.project
                        if model == 'project.project':
                            _logger.info(f"Applying privacy_visibility filter for {model}")
                            
                            # Check if user is employee
                            is_employee = env.user.has_group('base.group_user')
                            
                            current_domain = kwargs.get('domain', [])
                            if not isinstance(current_domain, list):
                                current_domain = []
                            ...
                            current_domain.extend(privacy_domain)
                            kwargs['domain'] = current_domain
                            _logger.info(f"Applied privacy_visibility filter. New domain: {kwargs['domain']}")
                        
                        # Standard follower filter for other models
                        elif 'message_follower_ids' in model_for_check.fields_get():
                            follower_domain = [('message_follower_ids.partner_id', '=', user_partner_id)]
                            
                            current_domain = kwargs.get('domain', [])
                            if not isinstance(current_domain, list):
                                current_domain = []
                            
                            current_domain.extend(follower_domain)
                            kwargs['domain'] = current_domain
                            _logger.info(f"Applied follower filter. New domain: {kwargs['domain']}")
                        else:
                            _logger.warning(f"Model {model} does not have 'message_follower_ids' field. Skipping follower filter.")
                    except Exception as e:
                        _logger.error(f"Error applying follower filter in _handle_rpc: {e}")
    ...
    def _handle_update(self, data):
        ...
        try:
            ...
            env = request.env(user=uid)
            user = env.user  # Get user record
            user_partner_id = user.partner_id.id if user.partner_id else None  # Get user's partner ID
            ...
            for model_name, model_config in body.items():
                ...
                if limit is not None and total_result_objects >= limit:
                    limit_reached = True
                    _logger.info(
                        f"🚫 LIMIT REACHED at model '{model_name}' - stopping processing (current: {total_result_objects}/{limit})")
                    break
                try:
                    ...
                    check_message_follower_ids = model_config.get('check_message_follower_ids', False)

                    if check_message_follower_ids and user_partner_id:
                        _logger.info(f"🔬 check_message_follower_ids is TRUE for model {model_name}")
                        ...
                        if model_name == 'project.project':
                            ...
                            _logger.info(f"Applying privacy_visibility filter: {privacy_domain}")
                            domain_filter.extend(privacy_domain)
                        
                        # Standard follower filter for other models
                        elif 'message_follower_ids' in model_obj.fields_get():
                            follower_domain = [('message_follower_ids.partner_id', '=', user_partner_id)]
                            _logger.info(f"Applying follower filter for user partner ID {user_partner_id}: {follower_domain}")
                            domain_filter.extend(follower_domain)
                        else:
                            _logger.warning(f"Model {model_name} does not have 'message_follower_ids' field. Skipping follower filter.")
                    ...
                    if not objects_list:
                        ...
                        try:
                            ...
                            if limit is not None:
                                ...
                                _logger.info(
                                    f"🎯 Applying search limit for '{model_name}': {search_limit} (remaining: {remaining_limit})")

                            # Execute search_read to get all records with limit
                            if search_limit:
                                all_records = model_obj.search_read(domain_filter, available_fields, limit=search_limit)
                            else:
                                all_records = model_obj.search_read(domain_filter, available_fields)
                            ...
                            if limit is not None and total_result_objects >= limit:
                                limit_reached = True
                                _logger.info(
                                    f"🎯 LIMIT REACHED after processing '{model_name}' ({total_result_objects}/{limit}) - stopping")
                                break
                    ...
                    # NEW LOGIC: Check for relation field changes (e.g., task_ids)
                    # If task_ids or other relation fields have changes, return complete project
                    relation_fields_to_check = []  # Will store relation fields like task_ids

                    # Scan fields_to_return for relation fields that appear in objects_list
                    for field_name in fields_to_return:
                        if '_ids' in field_name or '_id' in field_name:
                            relation_fields_to_check.append(field_name)

                    _logger.info(f"🔍 Detected relation fields to check: {relation_fields_to_check}")
                    ...
                    for obj_data in objects_list:
                        ...
                        if limit is not None and total_result_objects >= limit:
                            limit_reached = True
                            _logger.info(
                                f"🚫 LIMIT REACHED during object processing for '{model_name}' (current: {total_result_objects}/{limit}) - breaking")
                            break
                        try:
                            processed_count += 1
                            ...
                            # NEW: Check relation fields first
                            relation_changed = False
                            relation_change_details = []

                            for relation_field in relation_fields_to_check:
                                if relation_field in obj_data:
                                    payload_relations = obj_data.get(relation_field, [])

                                    # Convert to list if needed
                                    if isinstance(payload_relations, dict):
                                        payload_relations = [payload_relations]
                                    elif not isinstance(payload_relations, list):
                                        payload_relations = []

                                    _logger.info(
                                        f"🔄 Checking relation field '{relation_field}' for {model_name} ID {obj_id}")
                                    _logger.info(f"   Payload has {len(payload_relations)} relations")

                                    # Get current relations from Odoo
                                    try:
                                        odoo_record_check = model_obj.browse([obj_id]).exists()
                                        if odoo_record_check:
                                            odoo_relation_ids = getattr(odoo_record_check, relation_field, []).ids
                                            _logger.info(
                                                f"   Odoo has {len(odoo_relation_ids)} relations: {odoo_relation_ids}")

                                            # Extract IDs from payload
                                            payload_relation_ids = [r.get('id') for r in payload_relations if
                                                                    isinstance(r, dict) and 'id' in r]
                                            _logger.info(f"   Payload relation IDs: {payload_relation_ids}")

                                            # Check 1: Count changed?
                                            if len(payload_relation_ids) != len(odoo_relation_ids):
                                                relation_changed = True
                                                relation_change_details.append(
                                                    f"{relation_field}: count changed ({len(payload_relation_ids)} -> {len(odoo_relation_ids)})")
                                                _logger.info(f"   ✅ CHANGE DETECTED: Count changed!")

                                            # Check 2: IDs changed?
                                            elif set(payload_relation_ids) != set(odoo_relation_ids):
                                                relation_changed = True
                                                added = set(odoo_relation_ids) - set(payload_relation_ids)
                                                removed = set(payload_relation_ids) - set(odoo_relation_ids)
                                                relation_change_details.append(
                                                    f"{relation_field}: IDs changed (added: {added}, removed: {removed})")
                                                _logger.info(f"   ✅ CHANGE DETECTED: IDs changed!")

                                            # Check 3: For each relation in payload, check if write_date != create_date
                                            for payload_rel in payload_relations:
                                                if isinstance(payload_rel, dict):
                                                    rel_write_date = payload_rel.get('write_date')
                                                    rel_create_date = payload_rel.get('create_date')

                                                    if rel_write_date and rel_create_date:
                                                        # Simple string comparison (should be same format)
                                                        if rel_write_date != rel_create_date:
                                                            relation_changed = True
                                                            rel_id = payload_rel.get('id', 'unknown')
                                                            relation_change_details.append(
                                                                f"{relation_field}[{rel_id}]: write_date != create_date")
                                                            _logger.info(
                                                                f"   ✅ CHANGE DETECTED: Relation ID {rel_id} has write_date != create_date")

                                    except Exception as rel_check_error:
                                        _logger.error(
                                            f"   ❌ Error checking relation field '{relation_field}': {str(rel_check_error)}")

                            # If relation changed, force fetch complete object
                            if relation_changed:
                                _logger.info(f"🔥 RELATION CHANGES DETECTED for {model_name} ID {obj_id}:")
                                for detail in relation_change_details:
                                ...
                                try:
                                    ...
                                    if odoo_record:
                                        ...
                                        if full_record_data:
                                            ...
                                            updated_count += 1
                                            total_result_objects += 1
                                            _logger.info(
                                                f"🎯 Added object {obj_id} due to relation changes (total: {total_result_objects})")
                                        continue  # Skip normal write_date check
                                except Exception as fetch_error:
                                    _logger.error(f"❌ Error fetching object after relation change: {str(fetch_error)}")
                                ...
                                try:
                                    ...
                                    if time_diff_seconds > 1.0:
                                        ...
                                        if full_record_data:
                                            ...
                                            _logger.info(
                                                f"🎯 Added updated object {obj_id} to results (total result objects: {total_result_objects})")
                                    else:
                                        if time_diff_seconds <= -1.0:
                                            _logger.warning(
                        ...
                        try:
                            ...
                            if include_missing:
                                ...
                                try:
                                    ...
                                    missing_records = env[model_name].browse([])  # Initialize empty recordset
                                    if provided_ids:
                                        missing_domain_part = [('id', 'not in', provided_ids)]

                                        # Add active filter if the model supports it
                                        if 'active' in model_obj.fields_get():
                                            missing_domain_part.append(('active', '=', True))

                                        final_missing_domain = domain_filter + missing_domain_part
                                        ...
                                        if limit is not None:
                                            ...
                                            if remaining_limit <= 0:
                                                _logger.info(
                                                    f"🚫 LIMIT REACHED - no space for missing objects in '{model_name}'")
                                            else:
                                                missing_limit = remaining_limit
                                                _logger.info(
                                                    f"🎯 Limiting missing objects to {missing_limit} for '{model_name}'")

                                        if missing_limit is None or missing_limit > 0:
                                            _logger.info(f"Searching for missing records with domain: {final_missing_domain}")
                                            if missing_limit:
                                                missing_records = model_obj.search(final_missing_domain, limit=missing_limit)
                                            else:
                                                missing_records = model_obj.search(final_missing_domain)
                                    else:
    ...
    def config(self, **kwargs):
        ...
        try:
            ...
            return self._safe_json_response({
                ...
                "show": True,
                "form": {
                    # "baseUrl": "https://1cb3081e4eb6.ngrok-free.app",
                    "baseUrl": request.httprequest.host_url.rstrip('/'),
                    "wsUrl": "https://websocket.daisyconsulting.ma",
