                        ...
                        try:
                            ...
                            for relation_field in relation_fields_to_check:
                                if relation_field in obj_data:
                                    ...
                                    _logger.info(f"🔄 Checking relation field '{relation_field}' for {model_name} ID {obj_id}")
                                    ...
                                    try:
                                        ...
                                        if odoo_record_check:
                                            ...
                                            _logger.info(f"   Odoo has {len(odoo_relation_ids)} relations: {odoo_relation_ids}")
                                            
                                            # Extract IDs from payload
                                            payload_relation_ids = [r.get('id') for r in payload_relations if isinstance(r, dict) and 'id' in r]
                                            ...
                                            if len(payload_relation_ids) != len(odoo_relation_ids):
                                                ...
                                                relation_change_details.append(f"{relation_field}: count changed ({len(payload_relation_ids)} -> {len(odoo_relation_ids)})")
                                            ...
                                            # Check 2: IDs changed?
                                                ...
                                                relation_change_details.append(f"{relation_field}: IDs changed (added: {added}, removed: {removed})")
                                                ...
                                                if isinstance(payload_rel, dict):
                                                    ...
                                                    if rel_write_date and rel_create_date:
                                                        ...
                                                        if rel_write_date != rel_create_date:
                                                            ...
                                                            relation_change_details.append(f"{relation_field}[{rel_id}]: write_date != create_date")
                                                            _logger.info(f"   ✅ CHANGE DETECTED: Relation ID {rel_id} has write_date != create_date")
                                    
                                    except Exception as rel_check_error:
                                        _logger.error(f"   ❌ Error checking relation field '{relation_field}': {str(rel_check_error)}")
                                ...
                                try:
                                    ...
                                    if odoo_record:
                                        ...
                                        if full_record_data:
                                            ...
                                            _logger.info(f"🎯 Added object {obj_id} due to relation changes (total: {total_result_objects})")
    ...
    def config(self, **kwargs):
        ...
        try:
            ...
            return self._safe_json_response({
                ...
                "show": True,
                "form": {
                    "baseUrl": "https://95230d4f107c.ngrok-free.app",
